...
public class DFS_02_배열 {
	...
	static boolean ans;	//탈출 가능 여부 (못해 로 초기화)
...
//		(0,0) -> (N-1, N-1) 갈 수 있는지 확인
		//간혹 입구를 2, 출구를 3 주는 경우도 있더라
		//이 경우에 입력을 받으면서 미리 위치를 확보해 두는 경우도 있음
		for(int i=0; i<N; i++) {
			for(int j=0; j<N; j++) {
				map[i][j] = sc.nextInt();
			}
	...
	static void dfs(int r, int c) {
		...
		for(int i=0; i<4; i++) {
			...
			//위의 조건으로 3가지를 조건을 걸었는데 -> 벽처리 하면 하나로 끝낼 수도 있음
