...
//		Scanner sc = new Scanner(input1);
	...
	//start : 시작정점
	//dist, V, edges 이런 것도 static하게 만들지 않았다면.. 인자로 들고 있어야 한다.
	static void bellmanFord(int start) {
		...
		for(Edge e : edges) {
			if(dist[e.from] != INF && dist[e.to] > dist[e.from] + e.cost) {
				negativeCycle = true;	//갱신 일어났어!
				break;
			}
		}
		
		if(negativeCycle) System.out.println("음의 사이클이 있어서 무의미하다..");
		else {
