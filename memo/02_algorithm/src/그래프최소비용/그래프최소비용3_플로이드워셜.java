...
//		Scanner sc = new Scanner(input2);
		...
		for(int k = 0; k <V; k++) {
			for(int from=0; from<V; from++) {
				if(dist[from][k] == INF) continue; //출발지 -> 경유지 체크(무한이면 지금 못해~)
				for(int to=0; to<V; to++) {
					if(dist[k][to] == INF) continue;	//경유지 -> 도착지 체크. 연결 안되어있으면 의미없음
		...
		if(negativeCycle) System.out.println("음수사이클 존재함..");
		else {
			for(int[] tmp : dist)
